...
# Load all three CSVs
custom_data = read_csv('LinkedList_performance.csv')
...
for idx, method in enumerate(common_methods):
    ...
    ax = axes[idx]
    ax.plot(shared_sizes, custom_times, label='LinkedList', linestyle='-', linewidth=1.5)
...
plt.tight_layout()
plt.savefig('LinkedList_vs_CustomDoublyLinkedList_Comparisons.png', dpi=300, bbox_inches='tight')
plt.close()

print("Chart saved as 'LinkedList_vs_CustomDoublyLinkedList_Comparisons.png'")
