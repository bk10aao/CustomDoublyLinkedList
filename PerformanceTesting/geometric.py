...
# 1. Load the performance CSV datasets
custom_df = pd.read_csv('CustomDoublyLinkedList_performance.csv', sep=';')
jdk_df = pd.read_csv('LinkedList_performance.csv', sep=';')

# Robust filtering: ignore the 'Size' column and specific methods
exclude_keywords = {'size', 'removeall', 'retainall', 'sublist'}
methods = [
...
# Fix Y-axis to exactly match the data, removing top/bottom whitespace
...
ax.tick_params(colors='#ffffff', which='both', length=0) # Hide tick marks

for spine in ax.spines.values():
    spine.set_edgecolor('#555555')

# Use tight_layout instead of subplots_adjust
plt.tight_layout()
