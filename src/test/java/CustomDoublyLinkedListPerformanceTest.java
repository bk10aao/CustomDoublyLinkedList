...
public class CustomDoublyLinkedListPerformanceTest {
    ...
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter("CustomDoublyLinkedList_performance.csv")) {
            ...
            System.out.println("Performance data written to CustomDoublyLinkedList_performance.csv");
        } catch (IOException e) {
    ...
    private static long benchmarkConstructorCollection(Collection<Integer> input) {
        ...
        for (int i = 0; i < RUNS; i++) {
            long start = System.nanoTime();
            new CustomDoublyLinkedList<>(input);
            totalTime += System.nanoTime() - start;
        }
        return totalTime / RUNS;
    }
    ...
    private static long benchmarkEquals(int size) {
        long totalTime = 0;
        for (int i = 0; i < RUNS; i++) {
            CustomDoublyLinkedList<Integer> list1 = new CustomDoublyLinkedList<>();
            CustomDoublyLinkedList<Integer> list2 = new CustomDoublyLinkedList<>();
            for (int j = 0; j < size; j++) {
                list1.add(j);
                list2.add(j);
            }
